<template>
  <v-container>
    <v-btn v-if="getPrivilege == 2" right color="primary" v-on:click="goToCreateTicket">
      Create Ticket
    </v-btn>
    <v-data-table
        :headers="headers"
    ...
    >
      <template v-slot:item.state="{ item }">
        <v-avatar size="20"
                  v-bind:color="item.state != states[0] ? (item.state != states[1] ? 'green' : 'yellow') : 'red'">
        </v-avatar>
      </template>
    ...
    <v-dialog v-model="dialogSetTicketState">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" text @click="setTicketState">
            Update
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="dialogAssignTicket">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" text @click="setTicketAssign">
            Assign
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default class TicketList extends Vue {
  ...
  setTicketState() {
    ...
    this.$apollo.getClient().mutate({
      ...
      variables: {id: this.ticket.ID, input: {state: this.state, assignedTo: this.ticket.assignedToID}}
    }).then((res) => {
      console.log(res);
  ...
  setTicketAssign() {
    ...
    this.$apollo.getClient().mutate({
      mutation: ASSIGN_TICKET_MUTATION,
      variables: {id: this.ticket.ID, input: {state: this.ticket.state, assignedTo: this.assignedToID}}
    }).then((res) => {
      console.log(res);
