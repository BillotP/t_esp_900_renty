...
func TestMutationResolver_LoginAsCompany(t *testing.T) {
	var (
		...
		passwordHash string
		output       struct {
			LoginAsCompany models.Credential
		}
	...
	errBadCredentials = errors.New("[{\"message\":\"bad password provided\",\"path\":[\"loginAsCompany\"]}]")
	query = `mutation loginAsCompany($input: UserInput){loginAsCompany(input: $input){user{ID,username}}}`
	input = &models.UserInput{
	...
	t.Run("should login company correctly", func(t *testing.T) {
		...
		require.Equal(t, &expectedId, output.LoginAsCompany.User.ID)
		require.Equal(t, "thalesadmin", output.LoginAsCompany.User.Username)
	})
