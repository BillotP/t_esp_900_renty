...
func (r *QueryResolver) Anomalies(ctx context.Context) ([]*models.Anomaly, error) {
	...
	if err = r.DB.Joins("User").Where("username = ?", username).First(&tenant).Error; err == nil {
		if err = r.DB.Preload(clause.Associations).Preload("AssignedTo.User").Preload("CreateBy.User").Where("create_by_id = ?", tenant.ID).Find(&anomalies).Error; err != nil {
	...
	if err = r.DB.Joins("User").Where("username = ?", username).First(&estateAgent).Error; err == nil {
		if err = r.DB.Preload(clause.Associations).Preload("AssignedTo.User").Preload("CreateBy.User").Where("assigned_to_id = ?", estateAgent.ID).Find(&anomalies).Error; err != nil {
			lib.LogError("mutation/GetAnomalies", err.Error())
			return nil, err
		}
		return anomalies, nil
